 ...
 * @date 2019/3/12
 */
@Service("ImportEncryptionDataService")
@Transactional
public class ImportEncryptionDataImpl {
    ...
    public String execute(HttpServletRequest request) {
        ...
        String encryptionkey = request.getParameter("encryptionKey");//加密秘钥
        ...
        if (phones.size() > 0) {
            ...
            for (int i = 0; i < phones.size(); i++) {
                ...
                for (int j = 0; j < keys.length; j++) {
                    ...
                    CustomerLabel label = customerLabelDao.getCustomerLabelByName(k, custId);
