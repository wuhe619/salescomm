...
public class UserRoleDao extends SimpleHibernateDao<User, Serializable> {
    ...
    public void insert(UserRoles userRoles) {
        ...
        if (roles != null && user != null && !roles.isEmpty() && user.getId() != null) {
            for (RoleDTO role : roles) {
                this.executeUpdateSQL("insert into t_user_role_rel(ID,ROLE,OPTUSER,CREATE_TIME)" +
                        " VALUES(?,?,?,now())", user.getId(), role.getKey(), optUser);
            }
        }
    }
    ...
    public void delete(UserRoles userRoles) {
        ...
        if (roles != null && !roles.isEmpty()) {
            ...
            if (user == null || user.getId() == null) {
                ...
                this.executeUpdateSQL("delete from t_user_role_rel where ID=? and ROLE in(" +
                        sb.substring(0, sb.length() - 1) + ")", user.getId());
            }
        } else {
    ...
    public UserRoles getObj(UserRoles userRoles) {
        try {
            List list = this.sqlQuery("SELECT rel.ID,rel.ROLE,rel.LEVEL,rel.OPTUSER,rel.CREATE_TIME,r.NAME" +
                    " from t_role r,t_user_role_rel rel where rel.ROLE=r.ID and rel.ID=?", userRoles.getUser().getKey());
    ...
    public void delete(Long operateUserId, Long userId) throws SQLException {
        ...
        this.executeUpdateSQL(builder.toString(), userId, operateUserId);
    }
