...
//                sql.append(" and status =" + params.getInteger("status"));
    ...
    public Map<String, Object> getTotalFee(String SUBSCRIBER_ID, String callMonth, String apiId, String startDate, String endDate) {
        ...
        List param = new ArrayList();
        StringBuffer totalFeeSql = new StringBuffer("select sum(charge) /10000 totalCharge from am_charge_" + callMonth +
...
//            List<Map<String,Object>> dataList = list.getList();
            Object collect = list.getList().stream().map(m -> {
                ...
                String successSql = "SELECT count(*) monthSuccess FROM am_charge_" + params.getString("callMonth") + " WHERE length(response_msg)!='0' " +
                        "AND response_msg->>'$.cost'='1'  AND response_msg->>'$.rs'='" + map.get("rsId") + "'";
                Map<String, Object> successMap = jdbcTemplate.queryForMap(successSql);
                if (successMap != null) {
                    map.put("monthSuccess", String.valueOf(successMap.get("monthSuccess")));
                }
                return map;
            }).collect(Collectors.toList());
    ...
    public void getPersentByApi(Map map){
        ...
        String sql=" select percent,(select resname from t_market_resource where t.resource_Id=carp.resounse_id) resName from customer_api_resouse_precent carp where api_id=? and customer_id=? ";
        ...
        if(mapList==null||mapList.size()==0){
            ...
            if(property!=null&& StringUtils.isNotEmpty(property.getPropertyValue())){
                ...
                for(JSONObject jsonObject:jsonObjects){
                    ...
                    String rssql="select resname,resource_Id from t_market_resource where t.resource_Id=?";
