...
import com.bdaim.slxf.dto.TransactionEnum;
...
public class TransactionService {
    ...
    public List<Map<String, Object>> listTransactionsByCondtion(PageParam page, String customerId, TransactionQryParam param) throws Exception {
        ...
        if (type > 0 && type != 7) {
            StringBuilder sqlSumAmount = new StringBuilder("SELECT\n" +
                    "  SUM(t.amount) as sumAmount\n" +
                    " FROM\n" +
                    "  stat_bill_month c where 1=1 ");
            sqlAppendFuc(sqlSumAmount, customerId, type, startTime, params);
            logger.debug(sqlSumAmount.toString());
            int sumAmount = jdbcTemplate.queryForInt(sqlSumAmount.toString());
            //分转化为元
            if (type == 1) {//充值
