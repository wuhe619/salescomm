...
//                      int length = hBaseInputStream.available();
     ...
     * @return
     */
    public int cluePush(int pushType, String superId, String custId, String userId, String customerGroupId, String marketTaskId, int type) {
        // 邮件
        if (pushType == 2) {
            String phone = phoneService.getPhoneBySuperId(superId);
            ...
            if (type == 2) {
                ...
                String sql = "SELECT touch_id FROM t_touch_voice_log_" + nowTime.format(YYYYMM) + " WHERE called_duration >0 AND superid = ? customer_group_id = ? " +
                        " union all SELECT touch_id FROM t_touch_voice_log_" + nowTime.minusMonths(1).format(YYYYMM) + " WHERE called_duration >0 AND superid = ? customer_group_id = ? ";
            ...
            if (success) {
                ...
                try {
                    ...
                    LOG.info("众麦线索微信通知邮箱模板标题:{}内容{}", title, content);
                    ...
                    LOG.error("线索邮件userId:{}接收人:{},标题:{},内容:{},发送异常", userId, email.getPropertyValue().split(","), title, content, e);
                    return -1;
                }
            }
        }
