...
public class LkCrmAdminRoleDao extends SimpleHibernateDao<LkCrmAdminRoleEntity, Integer> {
    ...
    public int deleteRole(Integer roleId) {
        String sql = "   delete from 72crm_admin_role where role_id = ?";
        return executeUpdateSQL(sql, roleId);
    }

    public List<Map<String, Object>> getRoleListByRoleType(Integer roleType) {
        String sql = " select role_id as id ,role_name as title,role_name as remark,data_type as type,status,role_type as pid,label from 72crm_admin_role WHERE role_type=? and is_hidden = 1";
    ...
    public List<Map<String, Object>> getMenuIdsList(Integer role_id) {
        String sql = " select menu_id from 72crm_admin_role_menu where role_id = ?";
