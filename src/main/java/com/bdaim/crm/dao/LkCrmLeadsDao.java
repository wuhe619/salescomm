...
public class LkCrmLeadsDao extends SimpleHibernateDao<LkCrmLeadsEntity, Integer> {
    ...
    public Page pageCluePublicSea(int pageNum, int pageSize, long seaId, String search) {
        ...
        conditions.append(" z.* FROM t_customer_sea_list_" + seaId + " AS custG LEFT JOIN (SELECT max( IF ( (`a`.`name` = '客户级别'), `a`.`value`, NULL ) ) AS `客户级别`, max( IF ( (`a`.`name` = '跟进状态'), `a`.`value`, NULL ) ) AS `跟进状态`, max( IF ( ( `a`.`name` = '当前负责人' ), `a`.`value`, NULL ) ) AS `当前负责人`, max( IF ( ( `a`.`name` = '最新跟进时间' ), `a`.`value`, NULL ) ) AS `最新跟进时间`, max( IF ( (`a`.`name` = '微信'), `a`.`value`, NULL ) ) AS `weChat`, max( IF ( (`a`.`name` = '线索来源'), `a`.`value`, NULL ) ) AS `线索来源`, max( IF ( (`a`.`name` = '省市'), `a`.`value`, NULL ) ) AS `省市`, max( IF ( (`a`.`name` = '部门名称'), `a`.`value`, NULL ) ) AS `dept`, max( IF ( (`a`.`name` = '职位'), `a`.`value`, NULL ) ) AS `职位`, max( IF ( (`a`.`name` = 'QQ'), `a`.`value`, NULL ) ) AS `QQ`, max( IF ( (`a`.`name` = '邮箱'), `a`.`value`, NULL ) ) AS `邮箱`, max( IF ( (`a`.`name` = '网址'), `a`.`value`, NULL ) ) AS `site`, max( IF ( (`a`.`name` = '前负责人'), `a`.`value`, NULL ) ) AS `preUserId`, max( IF ( ( `a`.`name` = '进入公海次数' ), `a`.`value`, NULL ) ) AS `进入公海次数`, max( IF ( ( `a`.`name` = '未跟进天数' ), `a`.`value`, NULL ) ) AS `未跟进天数`, max( IF ( ( `a`.`name` = '剩余回收时间' ), `a`.`value`, NULL ) ) AS `剩余回收时间`, max( IF ( (`a`.`name` = '线索标签'), `a`.`value`, NULL ) ) AS `线索标签`, max( IF ( (`a`.`name` = '邮件次数'), `a`.`value`, NULL ) ) AS `邮件次数`, max( IF ( (`a`.`name` = '致电次数'), `a`.`value`, NULL ) ) AS `致电次数`, max( IF ( (`a`.`name` = '短信次数'), `a`.`value`, NULL ) ) AS `短信次数`, max( IF ( (`a`.`name` = '客户行业'), `a`.`value`, NULL ) ) AS `客户行业`, `a`.`batch_id` AS `field_batch_id` FROM ( `lkcrm_admin_fieldv` `a` JOIN `lkcrm_admin_field` `d` ON ( ( `a`.`field_id` = `d`.`field_id` ) ) ) WHERE ( (`d`.`label` = 11) AND (`a`.`batch_id` IS NOT NULL) AND (`a`.`batch_id` <> '') AND (`d`.`field_type` = 0) ) GROUP BY `a`.`batch_id`) AS z ON custG.id = z.field_batch_id WHERE custG.status =1 ");
    ...
    public List<Map<String, Object>> getPublicSeaClue(long seaId, String id) {
        String fieldSql = "SELECT max( IF ( (`a`.`name` = '客户级别'), `a`.`value`, NULL ) ) AS `客户级别`, max( IF ( (`a`.`name` = '跟进状态'), `a`.`value`, NULL ) ) AS `跟进状态`, max( IF ( ( `a`.`name` = '当前负责人' ), `a`.`value`, NULL ) ) AS `当前负责人`, max( IF ( ( `a`.`name` = '最新跟进时间' ), `a`.`value`, NULL ) ) AS `最新跟进时间`, max( IF ( (`a`.`name` = '微信'), `a`.`value`, NULL ) ) AS `微信`, max( IF ( (`a`.`name` = '线索来源'), `a`.`value`, NULL ) ) AS `线索来源`, max( IF ( (`a`.`name` = '省市'), `a`.`value`, NULL ) ) AS `省市`, max( IF ( (`a`.`name` = '部门名称'), `a`.`value`, NULL ) ) AS `部门名称`, max( IF ( (`a`.`name` = '职位'), `a`.`value`, NULL ) ) AS `职位`, max( IF ( (`a`.`name` = 'QQ'), `a`.`value`, NULL ) ) AS `QQ`, max( IF ( (`a`.`name` = '邮箱'), `a`.`value`, NULL ) ) AS `邮箱`, max( IF ( (`a`.`name` = '网址'), `a`.`value`, NULL ) ) AS `网址`, max( IF ( (`a`.`name` = '前负责人'), `a`.`value`, NULL ) ) AS `前负责人`, max( IF ( ( `a`.`name` = '进入公海次数' ), `a`.`value`, NULL ) ) AS `进入公海次数`, max( IF ( ( `a`.`name` = '未跟进天数' ), `a`.`value`, NULL ) ) AS `未跟进天数`, max( IF ( ( `a`.`name` = '剩余回收时间' ), `a`.`value`, NULL ) ) AS `剩余回收时间`, max( IF ( (`a`.`name` = '线索标签'), `a`.`value`, NULL ) ) AS `线索标签`, max( IF ( (`a`.`name` = '邮件次数'), `a`.`value`, NULL ) ) AS `邮件次数`, max( IF ( (`a`.`name` = '致电次数'), `a`.`value`, NULL ) ) AS `致电次数`, max( IF ( (`a`.`name` = '短信次数'), `a`.`value`, NULL ) ) AS `短信次数`, max( IF ( (`a`.`name` = '客户行业'), `a`.`value`, NULL ) ) AS `客户行业`, `a`.`batch_id` AS `field_batch_id` FROM ( `lkcrm_admin_fieldv` `a` JOIN `lkcrm_admin_field` `d` ON ( ( `a`.`field_id` = `d`.`field_id` ) ) ) WHERE ( (`d`.`label` = 11) AND (`a`.`batch_id` IS NOT NULL) AND (`a`.`batch_id` <> '') AND (`d`.`field_type` = 0) ) GROUP BY `a`.`batch_id` ";
