...
public class OaBackLogService {
    ...
    public R backLogNum() {
        ...
        Integer eventNum = eventDao.queryForInt(sql1, userId, userId, BaseUtil.getCustId());
        String sql2 = "select count(*) from lkcrm_task where status = 1 and " +
                "(main_user_id = ? or owner_user_id like CONCAT('%,',?,',%')) AND cust_id = ? ";
        Integer taskNum = eventDao.queryForInt(sql2, userId, userId, BaseUtil.getCustId());
        ...
        Integer announcementNum = eventDao.queryForInt(sql3, userId, BaseUtil.getCustId());
        ...
        Integer logNum = eventDao.queryForInt(sql4, userId, userId, BaseUtil.getUser().getDeptId(), BaseUtil.getCustId());
        ...
        Integer examineNum = eventDao.queryForInt(sql5, userId, BaseUtil.getCustId());
