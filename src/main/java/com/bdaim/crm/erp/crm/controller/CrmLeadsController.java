...
public class CrmLeadsController extends BasicAction {
    ...
    public ResponseJson deleteFiled(@RequestBody CustomerSeaSearch param) {
        ResponseJson responseJson = new ResponseJson();
        String sql = " ALTER TABLE `lkcrm_admin_field` ADD INDEX `index_label` (`label`), ADD INDEX `index_field_type` (`field_type`) ;";
        int data = crmAdminFieldDao.executeUpdateSQL(sql);
        sql = " ALTER TABLE `lkcrm_admin_user` ADD INDEX `index_custId` (`cust_id`) ; ";
