...
public class BiCustomerService {
    ...
    public R customerRecordInfo(Integer deptId, Long userId, String type, String startTime, String endTime) {
        ...
        String[] userIdsArr = userIds.split(",");
        for (int i = 1; i <= userIdsArr.length; i++) {
            sqlStringBuffer.append("select b.realname,IFNULL(count(a.record_id),0) as recordCount,IFNULL(count(DISTINCT a.types_id),0) as customerCount from lkcrm_admin_record as a left join lkcrm_admin_user as b on a.create_user_id = b.user_id where DATE_FORMAT(a.create_time,'")
    ...
    public R customerRecodCategoryStats(Integer deptId, Long userId, String type, String startTime, String endTime) {
        ...
        Integer finalTime = record.getInt("finalTime");
        StringBuffer sqlStringBuffer = new StringBuffer();
        sqlStringBuffer.append("select category,IFNULL(count(record_id),0) as recordNum,IFNULL(count(record_id)*100/(select count(*) from lkcrm_admin_record where DATE_FORMAT(create_time,'")
                .append(sqlDateFormat).append("') between '").append(beginTime).append("' and '").append(finalTime).append("' and create_user_id in (")
                .append(userIds).append(")),0) as proportion from lkcrm_admin_record where (DATE_FORMAT(create_time,'").append(sqlDateFormat)
    ...
    public R customerConversionStats(Integer deptId, Long userId, String type, String startTime, String endTime) {
        ...
        StringBuffer sqlStringBuffer = new StringBuffer();
        for (int i = 1; i <= cycleNum; i++) {
            sqlStringBuffer.append("select '").append(beginTime).append("' as type,IFNULL((select count(customer_id) from lkcrm_crm_customer where DATE_FORMAT(create_time,'")
                    .append(sqlDateFormat).append("') = '").append(beginTime).append("' and owner_user_id in (").append(userIds)
                    .append(")),0) as customerNum,IFNULL(count(a.customer_id)*100/(select count(customer_id) from lkcrm_crm_customer where DATE_FORMAT(create_time,'")
                    .append(sqlDateFormat).append("') = '").append(beginTime).append("' and owner_user_id in (").append(userIds)
                    .append(")),0) as pro,IFNULL(count(a.customer_id),0) as dealCustomerNum from lkcrm_crm_customer as a left join lkcrm_crm_contract as b on a.customer_id = b.customer_id where DATE_FORMAT(a.create_time,'")
    ...
    public R employeeCycleInfo(Integer deptId, Long userId, String type, String startTime, String endTime) {
        ...
        for (int i = 1; i <= userIdsArr.length; i++) {
            sqlStringBuffer.append("select a.realname,IFNULL(AVG(TIMESTAMPDIFF(DAY,b.create_time,c.order_date)),0) as cycle, count(b.customer_id) " +
                    ...
                    "lkcrm_crm_contract as c on b.customer_id = c.customer_id where DATE_FORMAT(b.create_time,'").append(sqlDateFormat)
