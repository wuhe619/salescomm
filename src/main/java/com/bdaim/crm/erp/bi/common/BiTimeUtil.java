...
public class BiTimeUtil {
    ...
    @Autowired
    private LkCrmAdminUserDao adminUserDao;
    ...
    public Record analyzeType(Record record) {
        ...
        if (userId != null) {
            ...
            List<Record> records = adminDeptService.queryDeptByParentDept(deptId,
                    BaseConstant.AUTH_DATA_RECURSION_NUM);
            List<Integer> deptIds = new ArrayList<>();
            deptIds.add(deptId);
            records.forEach(dept -> deptIds.add(dept.getInt("id")));
            List<Long> userIdList = adminUserDao.queryUserIdByDeptId(SqlAppendUtil.sqlAppendWhereIn(deptIds));
