...
public class CustomerSeaService {
    ...
    public Page page(CustomerSeaParam param, int pageNum, int pageSize) {
        ...
        if (page.getData() != null && page.getData().size() > 0) {
            ...
            if ("publicSea".equals(param.getType())) {
                statSql = "SELECT COUNT(status=0 OR null) sumCount,IFNULL(COUNT(super_data like ''%\"SYS007\":\"未跟进\"%'' AND status = 0 OR null),0) AS noFollowSum, IFNULL(COUNT(distinct(super_data->>''$.SYS014'')OR null),0) AS clueSurplusSum, IFNULL(COUNT(`call_fail_count` >= 1 OR null),0) AS failCallSum FROM " + ConstantsUtil.SEA_TABLE_PREFIX + "{0} WHERE 1=1 '";
            } else {
    ...
    public int saveImportData(String custId, long userId, String userType, long seaId, String fileName, JSONArray headers) {
        // 查询公海下默认客群
        CustomerSeaProperty csp = customerSeaDao.getProperty(String.valueOf(seaId), "defaultClueCgId");
        String cgId = null;
        if (csp == null) {
            LOG.warn("公海:{},默认线索客群不存在,开始创建", seaId);
            cgId = createDefaultClueCGroup0(seaId, "公海默认客群", custId);
        } else {
            cgId = csp.getPropertyValue();
        }
        if (StringUtil.isEmpty(cgId)) {
            LOG.warn("公海:{},默认线索客群不存在", seaId);
            return -1;
        }
        List<String> headerList = JSONObject.parseArray(headers.toJSONString(), String.class);
        // 异步处理客户群数据
        String finalCgId = cgId;
        new Thread(() -> {
     ...
     * @return
     */
    public Page pagePublicClue(CustomerSeaSearch param) {
        Page page;
        StringBuffer sb = new StringBuffer();
        CustomerSea customerSea = customerSeaDao.get(NumberConvertUtil.parseLong(param.getSeaId()));
        sb.append(" select custG.id, custG.user_id, custG.status, custG.call_count callCount, DATE_FORMAT(custG.last_call_time,'%Y-%m-%d %H:%i:%s') lastCallTime, custG.intent_level intentLevel,");
...
//        sb.append(" AND custG.status<>2 ");
     ...
     * @return
     */
    public Page pagePrivateClue(CustomerSeaSearch param) {
        Page page;
        String userId = String.valueOf(param.getUserId());
        StringBuffer sb = new StringBuffer();
        CustomerSea customerSea = customerSeaDao.get(NumberConvertUtil.parseLong(param.getSeaId()));
