...
public class BillDao extends SimpleHibernateDao {
    ...
    public double sumCustomerMonthProfit(String custId, String yearMonth) {
        double amount = 0.0;
        String sql = "select  CAST((sum(sb.amount/1000)-sum(sb.prod_amount/1000))-((sum(sb.amount/1000)-sum(sb.prod_amount/1000))*((select tp.property_value from t_customer_property tp where tp.cust_id=sb.cust_id and tp.property_name='commission_rate')/100))as decimal(64,3)) profit from stat_bill_month sb \n" +
