...
public class CustomGroupService {
   ...
   /* public Page listPage(String customer_group_id, Integer pageNum, Integer pageSize, String id) {
        ...
        try {
            ...
            String total = this.jdbcTemplate.queryForObject("select count(1) " + sql.toString());
            ...
            List<Map<String, Object>> list = this.customGroupDao.sqlQuery(sql_list);
            for (Map<String, Object> map : list) {
                if (map != null) {
    ...
    public Map<String, Object> countCustomerGroupCallDataV4(int timeType, String customerGroupId, LoginUser
        ...
        try {
            ...
            for (LocalDateTime nowTime = localStartDateTime; nowTime.isBefore(localEndDateTime); ) {
                ...
                //管理员查全部
                if ("1".equals(loginUser.getUserType())) {
                    callLogListTmp = this.customGroupDao.sqlQuery(sqlSb.toString(), loginUser.getCustId(), customerGroupId, startTime, endTime);
                } else {
                    // 组长查整个组的外呼统计
                    if ("1".equals(loginUser.getUserGroupRole())) {
                        List<CustomerUserDTO> customerUserDTOList = customerUserDao.listSelectCustomerUserByUserGroupId(loginUser.getUserGroupId(), loginUser.getCustId());
                        ...
                        callLogListTmp = this.customGroupDao.sqlQuery(sqlSb.toString(), loginUser.getCustId(), customerGroupId, startTime, endTime, loginUser.getId());
                    }
                }
                // 分月数据汇总到总数据中
            ...
            for (LocalDateTime nowTime = localStartDateTime; nowTime.isBefore(localEndDateTime); ) {
                ...
                successVoiceLogListTmp = this.customGroupDao.sqlQuery(sqlSb.toString(), loginUser.getCustId(), customerGroupId, startTime, endTime);
                // 分月数据汇总到总数据中
            ...
            for (LocalDateTime nowTime = localStartDateTime; nowTime.isBefore(localEndDateTime); ) {
                ...
                labelDataListTmp = customGroupDao.sqlQuery(sqlSb.toString(), loginUser.getCustId(), customerGroupId, startTime, endTime);
                // 分月数据汇总到总数据中
    ...
    /*public Map<String, Object> countCustomerGroupCallDataV5(int timeType, String customerGroupId, UserQueryParam
        ...
        try {
            ...
            callLogList = this.customGroupDao.sqlQuery(sqlSb.toString(), localStartDateTime.format(DateTimeFormatter.ofPattern("yyyy-MM-dd 00:00:00")),
    ...
    public Map<String, Object> countCGUserCallData(int timeType, String customerGroupId, UserQueryParam
        ...
        try {
            ...
            page = this.customGroupDao.sqlPageQuery(sqlSb.toString(), userQueryParam.getPageNum(), userQueryParam.getPageSize(), startTime, endTime, customerGroupId);
    ...
    public void exportCustomerGroupCallData(int timeType, String customerGroupId, UserQueryParam
        ...
        try (OutputStream outputStream = response.getOutputStream()) {
            ...
            sqlSb.append(" GROUP BY user_id ");
            List<Map<String, Object>> list = this.customGroupDao.sqlQuery(sqlSb.toString(), startTime,
            ...
            Map<String, Object> customerSingleSelectLabelName = new HashMap<>();
            // 处理标签ID或者标签明显
            List<Map<String, Object>> labelNames = customGroupDao.sqlQuery("SELECT label_name, label_id, type FROM t_customer_label WHERE status = 1 AND cust_id = ? ", userQueryParam.getCustId());
            for (Map<String, Object> map : labelNames) {
                if (map != null && map.get("label_name") != null) {
                    // 只存储单选类型的自建属性名称
                    if ("2".equals(String.valueOf(map.get("type")))) {
            ...
            for (LocalDateTime nowTime = localStartDateTime; nowTime.isBefore(localEndDateTime); ) {
                ...
                labelDataListTmp = customGroupDao.sqlQuery(sqlSb.toString(), customerGroupId, startTime, endTime);
                // 分月数据汇总到总数据中
                if (labelDataListTmp.size() > 0) {
                    labelDataList.addAll(labelDataListTmp);
                }
        ...
        try {
            ...
            if (excelHeads != null) {
                ...
                if (list.size() > 0) {
                    ...
                    int uCount = RedisUtil.getRedisUtil().batchSet(u);
//                    int uCount = customerDao.insertBatchUData(data);
   ...
   /* public JSONObject getCustomGroupList(String custId, String groupId, Integer pageNum, Integer pageSize) {
        ...
        try {
            ...
            String total = this.customGroupDao.queryForObject("select count(1) " + sql.toString());
