...
public class BatchDetailImpl implements BatchDetaiService {
    ...
    public PageList getDetailList(DetailQueryParam detailQueryParam, Long userId, String userType, JSONArray custProperty, String role) {
        ...
        sqlBuilder.append("SELECT t.* FROM ( select (select comp_id from nl_batch where id=custG.batch_id) AS custId,custG.label_seven AS bindId,custG.label_eight AS exTime,(concat(IFNULL(custG.label_nine,\"\"),(CASE WHEN custG.label_five IS NOT NULL THEN CONCAT(custG.label_five,\",\") else  \"\" end ))) AS midleNumber,(case when custG.label_eight is not null then (case when STR_TO_DATE(custG.label_eight, '%Y-%m-%d %H:%i:%s')<now() then '有效' else '已过期' end) else '' end) effectStatus,custG.id,custG.batch_id,custG.enterprise_id,custG.id_card,custG.`status`,custG.channel,custG.user_id, t.realname,GROUP_CONCAT(t2.label_id) AS labelId,\n" +
